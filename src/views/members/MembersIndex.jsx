...
export default function MembersIndex() {
  ...
  const getRecords = async (url = "/members", keyword = "") => {
    setLoading(true);
    try {
  ...
  const onDelete = () => {
    ...
    axiosClient
      .delete(`/members/${selectedRecord.id}`)
      .then(() => {
    ...
    <div className="mt-5">
      <div className="flex flex-row mb-5">
        <div className="basis-1/2 my-auto">
          <PageTitle classes={"mb-0"} title={"Miembros"} />
        </div>
            ...
            <table className="w-full table-auto border-collapse border">
              <thead>
                <tr className="border-b text-gray-700 font-bold">
                  ...
                  <th className="py-3 border text-center">Plan</th>
              ...
              <tbody>
                {records.map((record, index) => (
                  <tr key={index} className="">
                    ...
                    <td className="py-1 px-3 border">{record.plan.name}</td>
                    <td className="py-1 px-3 border flex flex-wrap justify-center items-center">
                      <Link
                        to={`/members/edit/${record.id}`}
